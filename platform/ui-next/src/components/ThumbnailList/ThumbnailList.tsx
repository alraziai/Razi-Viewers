      ...
      >
        {thumbnailItems.length > 0 && (
          <div
            ...
            className="bg-[#0A1628] grid grid-cols-[repeat(auto-fit,_minmax(0,135px))] place-items-start gap-[4px]"
          >
        ...
        {listItems.length > 0 && (
          <div
            id="ohif-thumbnail-list"
            className="bg-[#0A1628] grid grid-cols-[repeat(auto-fit,_minmax(0,275px))] place-items-start gap-[2px]"
          >
            {listItems.map(item => {
              const { displaySetInstanceUID, componentType, numInstances, ...rest } = item;
              const isActive = activeDisplaySetInstanceUIDs.includes(displaySetInstanceUID);
